...
- **Tests:** `pytest tests/` (run via `uv run --with pytest pytest tests/` or the pytest binary at `~/.local/bin/pytest`; NOT in repo venv). Tests run against the live instance (default `PROJECTBASE_URL=http://127.0.0.1:8120`, superuser `f@flow.com` / `superdev123`). 381 tests across 26 files.
...
app/
  ...
  pb_hooks/             <- PocketBase backend logic (onRecordCreate/Update, routerAdd)
    ...
    90_agents.pb.js / 91_mcp_server.pb.js / 92_agent_collaboration.pb.js / 93_swarm_choreography_dag.pb.js / 94_workspace_synthesis.pb.js / 95_federation_analytics.pb.js / 96_git_workspace_engine.pb.js / 97_autoscale_workload_engine.pb.js / 98_cluster_replication_engine.pb.js / 99_webhook_automation_engine.pb.js / 100_sdk_observability_engine.pb.js / 101_consensus_gate_engine.pb.js / 102_sso_rbac_engine.pb.js / 103_workflow_automations_engine.pb.js / 104_multi_tenant_quota_engine.pb.js / 105_auto_heal_pipeline.pb.js <- agent bridge + FastMCP + leases & telemetry + swarm DAG + workspace synthesis + federation & analytics + git workspace engine + workload autoscaler & self-healing + cross-cluster replication & edge sync + webhook automation & DLQ + openapi sdk generator & webhook observability + consensus gates + SSO federation & granular RBAC matrix + workflow automations & AI agent trigger pipelines + multi-tenant isolation & resource quotas + autonomous agent auto-healing & self-remediation pipeline
...
tests/                 <- test_api.py, test_selfhosting.py, test_autonomous_runner_sync.py, test_foss_schema.py, test_css_sync.py,
                          test_bulk_actions.py, test_issue_relations.py, test_deploy_consistency.py, test_benchmarks.py, test_openapi_drift.py,
                          test_secret_scan.py, test_agents_drift_guard.py, test_fixture_hygiene.py, test_swarm_dag.py, test_workspace_synthesis.py,
                          test_federation_analytics.py, test_git_workspace_engine.py, test_autoscale_orchestration.py, test_cluster_replication.py,
                          test_webhook_automation.py, test_sdk_observability.py, test_consensus_gates.py, test_sso_rbac_matrix.py, test_workflow_automations.py, test_multi_tenant_quotas.py, test_auto_heal_pipeline.py
...
`users` (auth) · `projects` (multi-project, `custom_field_defs`) · `issues` (status, priority, estimate, project rel, assignee, subtasks JSON, milestone) · `cycles` (sprints + burndown) · `milestones` (North Star roadmap) · `labels` · `comments` · `activity` (audit log) · `notifications` (in-app inbox: recipient, issue, type, read) · `workflow_rules` · `workflow_runs` · `workflow_triggers_audit` · `tenants` · `tenant_quotas` · `tenant_memberships`.

## 4. Custom API & MCP

- Custom routes in `app/pb_hooks/30_custom_routes.pb.js`, `103_workflow_automations_engine.pb.js` & `104_multi_tenant_quota_engine.pb.js`: `/api/projectbase/health`, `/version`, `/stats`, `/quick-task`, `/api/projectbase/automations/*`, `/api/projectbase/tenants/*`.

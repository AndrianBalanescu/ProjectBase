...
- **Tests:** `pytest tests/` (run via `uv run --with pytest pytest tests/` or the pytest binary at `~/.local/bin/pytest`; NOT in repo venv). Tests run against the live instance (default `PROJECTBASE_URL=http://127.0.0.1:8120`, superuser `f@flow.com` / `superdev123`). 299 tests across 18 files.
...
app/
  ...
  pb_hooks/             <- PocketBase backend logic (onRecordCreate/Update, routerAdd)
    ...
    90_agents.pb.js / 91_mcp_server.pb.js / 92_agent_collaboration.pb.js / 93_swarm_choreography_dag.pb.js / 94_workspace_synthesis.pb.js / 95_federation_analytics.pb.js / 96_git_workspace_engine.pb.js / 97_autoscale_workload_engine.pb.js <- agent bridge + FastMCP + leases & telemetry + swarm DAG + workspace synthesis + federation & analytics + git workspace engine + workload autoscaler & self-healing
...
tests/                 <- test_api.py, test_selfhosting.py, test_autonomous_runner_sync.py, test_foss_schema.py,
                          ...
                          test_git_workspace_engine.py, test_autoscale_orchestration.py (Epic 14 workload autoscaling & self-healing)

...
- **Tests:** `pytest tests/` (run via `uv run --with pytest pytest tests/` or a user-local pytest install (e.g. `~/.local/bin/pytest` via `pip install --user pytest`)). Tests run against the live instance (default `PROJECTBASE_URL=http://127.0.0.1:8120`, superuser `f@flow.com` / `superdev123`). 559 tests across 48 files.
...
app/
  ...
  pb_hooks/             <- PocketBase backend logic (onRecordCreate/Update, routerAdd)
    ...
    90_agents.pb.js / 91_mcp_server.pb.js / 92_agent_collaboration.pb.js / 93_swarm_choreography_dag.pb.js / 94_workspace_synthesis.pb.js / 95_federation_analytics.pb.js / 96_git_workspace_engine.pb.js / 97_autoscale_workload_engine.pb.js / 98_cluster_replication_engine.pb.js / 99_webhook_automation_engine.pb.js / 100_sdk_observability_engine.pb.js / 101_consensus_gate_engine.pb.js / 102_sso_rbac_engine.pb.js / 103_workflow_automations_engine.pb.js / 104_multi_tenant_quota_engine.pb.js / 105_auto_heal_pipeline.pb.js / 106_semantic_brain_engine.pb.js / 107_session_ingestion_engine.pb.js / 108_session_observability_engine.pb.js / 109_session_branching_intervention_engine.pb.js / 110_session_trajectories_swarm_engine.pb.js / 111_session_merges_conflict_engine.pb.js / 112_fleet_budget_quota_engine.pb.js / 113_agent_evaluation_benchmark_engine.pb.js / 114_ephemeral_sandbox_orchestrator.pb.js / 115_incident_warroom_engine.pb.js / 116_knowledge_graph_engine.pb.js / 117_code_review_swarm_engine.pb.js / 118_release_flight_control_engine.pb.js / 119_security_sentinel_engine.pb.js / 120_tdd_mutation_engine.pb.js / 121_agent_time_travel_debugger.pb.js / 122_architecture_blast_radius_engine.pb.js / 123_profiler_flamegraph_engine.pb.js
...
tests/                 <- 559 tests across 48 files (test_agent_evaluations_engine.py, test_agent_time_travel_debugger.py, test_agents_drift_guard.py, test_api.py, test_architecture_blast_radius_engine.py, test_auto_heal_pipeline.py, test_autonomous_runner_sync.py, test_autoscale_orchestration.py, test_benchmarks.py, test_bulk_actions.py, test_cluster_replication.py, test_code_review_swarm_engine.py, test_consensus_gates.py, test_css_sync.py, test_custom_route_auth_guards.py, test_deploy_consistency.py, test_ephemeral_sandboxes_orchestrator.py, test_federation_analytics.py, test_fixture_hygiene.py, test_fleet_budget_quotas.py, test_foss_schema.py, test_git_workspace_engine.py, test_git_webhook_hmac.py, test_incident_warroom_engine.py, test_issue_relations.py, test_kanban_touch_dnd.py, test_knowledge_graph_engine.py, test_multi_tenant_quotas.py, test_openapi_drift.py, test_profiler_flamegraph_engine.py, test_release_flight_control_engine.py, test_sdk_observability.py, test_secret_scan.py, test_security_sentinel_engine.py, test_selfhosting.py, test_semantic_brain.py, test_session_branching_and_intervention.py, test_session_ingestion_engine.py, test_session_merges_conflict_engine.py, test_session_observability.py, test_session_trajectory_and_swarm.py, test_sso_rbac_matrix.py, test_swarm_dag.py, test_tdd_mutation_engine.py, test_webhook_automation.py, test_workflow_automations.py, test_workspace_synthesis.py, test_listview_sort_logic.py)
...
`users` (auth) · `projects` (multi-project, `custom_field_defs`) · `issues` (status, priority, estimate, project rel, assignee, subtasks JSON, milestone) · `cycles` (sprints + burndown) · `milestones` (North Star roadmap) · `labels` · `comments` · `activity` (audit log) · `notifications` (in-app inbox) · `workflow_rules` · `workflow_runs` · `tenants` · `tenant_quotas` · `semantic_embeddings` · `semantic_admission_policies` · `semantic_review_audit` · `agent_sessions` · `session_audits` · `session_interventions` · `session_trajectories` · `swarm_clusters` · `session_merges` · `merge_conflicts` · `budget_policies` · `token_quotas` · `cost_ledger_entries` · `budget_overrides` · `eval_suites` · `eval_runs` · `eval_metrics` · `eval_benchmarks` · `dev_sandboxes` · `sandbox_templates` · `sandbox_executions` · `sandbox_snapshots` · `incidents` · `incident_events` · `incident_hypotheses` · `incident_mitigations` · `incident_postmortems` · `knowledge_nodes` · `knowledge_relations` · `architectural_invariants` · `invariant_verifications` · `code_reviews` · `review_critiques` · `review_patches` · `merge_verdicts` · `tdd_suites` · `tdd_cases` · `mutation_runs` · `flaky_quarantines` · `debug_sessions` · `debug_trace_frames` · `debug_breakpoints` · `debug_state_snapshots`.

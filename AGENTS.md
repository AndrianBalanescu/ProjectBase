...
- **Tests:** `pytest tests/` (run via `uv run --with pytest pytest tests/` or the pytest binary at `/home/ubuntu/.local/bin/pytest`). Tests run against the live instance (default `PROJECTBASE_URL=http://127.0.0.1:8120`, superuser `f@flow.com` / `superdev123`). 418 tests across 30 files.
...
tests/                 <- 418 tests across 30 files (test_agents_drift_guard.py, test_api.py, test_auto_heal_pipeline.py, test_autonomous_runner_sync.py, test_autoscale_orchestration.py, test_benchmarks.py, test_bulk_actions.py, test_cluster_replication.py, test_consensus_gates.py, test_css_sync.py, test_deploy_consistency.py, test_federation_analytics.py, test_fixture_hygiene.py, test_foss_schema.py, test_git_workspace_engine.py, test_issue_relations.py, test_multi_tenant_quotas.py, test_openapi_drift.py, test_sdk_observability.py, test_secret_scan.py, test_selfhosting.py, test_semantic_brain.py, test_session_branching_and_intervention.py, test_session_ingestion_engine.py, test_session_observability.py, test_sso_rbac_matrix.py, test_swarm_dag.py, test_webhook_automation.py, test_workflow_automations.py, test_workspace_synthesis.py)
...
`users` (auth) · `projects` (multi-project, `custom_field_defs`) · `issues` (status, priority, estimate, project rel, assignee, subtasks JSON, milestone) · `cycles` (sprints + burndown) · `milestones` (North Star roadmap) · `labels` · `comments` · `activity` (audit log) · `notifications` (in-app inbox) · `workflow_rules` · `workflow_runs` · `tenants` · `tenant_quotas` · `semantic_embeddings` · `semantic_admission_policies` · `semantic_review_audit` · `agent_sessions` · `session_audits` · `session_interventions`.

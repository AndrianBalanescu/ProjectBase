...
- **Tests:** `pytest tests/` (run via `uv run --with pytest pytest tests/` or the pytest binary at `/home/ubuntu/.local/bin/pytest`). Tests run against the live instance (default `PROJECTBASE_URL=http://127.0.0.1:8120`, superuser `f@flow.com` / `superdev123`). 556 tests across 46 files.

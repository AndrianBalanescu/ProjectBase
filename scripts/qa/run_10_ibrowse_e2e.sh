...
declare -a GOALS=(
  ...
  "$LOGIN Open Roadmap from the primary navigation. Verify milestones render without blank content and milestone interactions remain usable."
  "$LOGIN Open Sessions and verify execution runs render. Then open API Docs and verify it loads /docs in a separate tab, not an SPA Docs view."
  ...
  "$LOGIN Inspect the top header bar: verify brand logo/name, project selector dropdown, search bar, the primary view buttons in exact order (Board, List, Cycles, Roadmap, Projects), plus clear Sessions access and an external API Docs link, and user avatar are visible. Press Ctrl+K (or Cmd+K) to open the Command Palette. Report findings."
)
